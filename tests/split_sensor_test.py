...
def callback2 (sender, data):
	...
	global outputted_value2
	
	outputted_value2 = convertToNumber(x,y)	

	print(f"TWO: {sender}: {int_values} / {outputted_value2}")
...
async def run(addr1, add2, loop):
	async with BleakClient(addr1, loop=loop) as client:
		...
		async with BleakClient(add2, loop=loop) as client2:
			...
			await client2.write_gatt_char(INPUT_COMMAND_UUID, bytearray([0x01,0x02,PORT_NUM2,0x23,0x00,0x01,0x00,0x00,0x00,0x02,0x01]), True)

			await client.start_notify(SENSOR_VAL_UUID, callback)
			await client2.start_notify(SENSOR_VAL_UUID, callback2)
			...
			global outputted_value, prev_value, outputted_value2, prev_value2, GLOBAL_Q
			
			try:
				while True:	
					await asyncio.sleep(0)

					if outputted_value == prev_value and outputted_value2 == prev_value2:
						continue
					...
					if not outputted_value == prev_value:
						...
						GLOBAL_Q.append( (client2, OUTPUT_COMMAND_UUID, bytearray([0x06,0x04,0x01,copy]) ) )
						GLOBAL_Q.append( (client2, OUTPUT_COMMAND_UUID, bytearray([0x02, 0x01, 0x01, translate_speed(  copy>0  ) ]) ) )
						GLOBAL_Q.append( (client, OUTPUT_COMMAND_UUID, bytearray([0x01, 0x01, 0x01, translate_speed(  (copy>0) * (-1 if copy<4 else 1) ) ]) ) )

					if not outputted_value2 == prev_value2:
						copy2 = outputted_value2

						GLOBAL_Q.append( (client, OUTPUT_COMMAND_UUID, bytearray([0x06,0x04,0x01,copy2]) ) )
